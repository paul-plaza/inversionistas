...
{
    internal sealed class EventsByOperationIdSpecs : Specification<Event, EventResponse>
    {
        public EventsByOperationIdSpecs(IEnumerable<int> operationsId, bool isNoTracking = false)
        {
            Query.Select(x => new EventResponse(
                ...
                x.ItemType.ToString(),
                x.EventDetails
                    .Where(y => y.Status == Status.Active && operationsId.Contains(y.OperationId))
